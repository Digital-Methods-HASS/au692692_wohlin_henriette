...
author: "Henriette Steinus Wohlin"
...
```{r Homicides In Europe over time, eval = TRUE}
ggplot(data = Western_Europe) + 
  geom_line(aes(x = Year, y = homicides_per_100k, color = Entity)) +
  labs(title = "Homicides In Europe over time",
       y = "Homicides per 100k",
       x = "Year") +
...
```{r Danish Monarchs and reign length, eval = TRUE}
Danish_monarchs <- read.csv2("data/danish_monarchs.csv")

Danish_monarchs <- Danish_monarchs %>% 
  mutate(
    midyear = end_of_reign. - (end_of_reign.- start_of_reign.)/2,
    reign_length2 = end_of_reign. - start_of_reign.)

ggplot(data = Danish_monarchs, aes(x = midyear, y = reign_length2)) +
  geom_smooth() +
  labs(title = "Danish rulers through time", x = "Midyear", y = "Reign duration in years") 
```

```{r How would you characterize the trend compared to the homicides above?}
#It does appear that as homicides went down in Scandinavia, the reign length increased for the danish monarchs - times became more peaceful. 
```
...
As we could see with danish monarchs and their reign over time, it would appear that we have become more civilized today. This can especially be seen in the "Homicides in Europe over time" as we can see that homicides have gone down drastically in the countries shown. 
